<?php include("conecta_facturacion.php"); ?>
...
//echo "Modo: ".$_session_start[emisor] ;

if ($_SESSION[emisor] == 'EKU9003173C9'){
	
	//echo " Timbrado de Prueba *";
$pacnomcor = "PRUEBAS";
$pacurl = "https://pruebas.timbracfdi33.mx/Timbrado.asmx?wsdl";
$pacusu = "mvpNUXmQfK8=";
...
Durante la etapa de pruebas deberás utilizar el siguiente endpoint:
https://pruebas.timbracfdi33.mx/Timbrado.asmx
Puedes hacer referencia a la definición del servicio dependiendo de el lenguaje de programación que utilices
https://pruebas.timbracfdi33.mx/Timbrado.asmx?WSDL
...
try {
 ...
 //	echo "<p>Error: " . $fault->faultcode . "-" . $fault->faultstring . " </p>";exit;   OJO AQUI ESTA EL ERROR CON MAS DETALLE
	exit;
}
if ($response->TimbraCFDIResult->anyType[4] == NULL) {
	...
	echo "
		...
		<p align='center' class='campo_requerido'>".trim( $response->TimbraCFDIResult->anyType[5] )."</p>
		<p align='center' class='campo_requerido'>".trim( $response->TimbraCFDIResult->anyType[6] )."</p>
	...
	<p align='center' class='campo_requerido'>".trim( $response->TimbraCFDIResult->anyType[3] )."</p>
	<p align='center' class='campo_requerido'>".trim( $response->TimbraCFDIResult->anyType[4] )."</p>
	<p align='center' class='campo_requerido'>".trim( $response->TimbraCFDIResult->anyType[5] )."</p>
	<p align='center' class='campo_requerido'>".trim( $response->TimbraCFDIResult->anyType[6] )."</p>
	<p align='center' class='campo_requerido'>".trim( $response->TimbraCFDIResult->anyType[7] )."</p>
	<p align='center' class='campo_requerido'>".print_r( $response )."</p>
      ";  
...
<table width="100%" border="0">
  ...
  <td align="center">&nbsp;</td>
	...
	$fecha_cer = $tfd['FechaTimbrado']; 
			...
			// Agrega la version
			$actualiza = mysqli_query($cone,"UPDATE `invoices` SET `version` = '4.0' 
			WHERE `invoices`.`id` = '$cve';");
	...
	$actualiza = mysqli_query($cone,"UPDATE `invoices` SET `cadena` = '$cadenaOriginal' WHERE `invoices`.`id` = '$cve';");
	$actualiza = mysqli_query($cone,"UPDATE `invoices` SET `sello_cfdi` = '$sello_c' WHERE `invoices`.`id` = '$cve';");
	$actualiza = mysqli_query($cone,"UPDATE `invoices` SET `sello_sat` = '$sello_s' WHERE `invoices`.`id` = '$cve';");
	$actualiza = mysqli_query($cone,"UPDATE `invoices` SET `certificado_sat` = '$cert_sat' WHERE `invoices`.`id` = '$cve';");
	$actualiza = mysqli_query($cone,"UPDATE `invoices` SET `rfc_prov` = '$rfc_prov' WHERE `invoices`.`id` = '$cve';");
	$actualiza = mysqli_query($cone,"UPDATE `invoices` SET `fecha_hora_cer` = '$fecha_cer' WHERE `invoices`.`id` = '$cve';");
				...
				echo " <p> Factura Timbrada Correctamente: ".$archi_xml."  </p> ";
				echo '<img src="https://iconos8.es/icon/HkMwI7YOsJzc/champagne;"/>';
 ...
 <!--<a href="../../detalle_invoices.php?idorden=<?php //echo $ar2[0]; ?>&serie=<?php //echo $ar2[0]; ?>">Ir a Factura</a> -->
...
<table width="100%" border="0">
      <tr>
        <td align="center"><a href="../../facturacion.php"></a></td>
      </tr>
</table>
<table width="72%" border="0" align="center">
  <tr>
    <td width="52%" align="center"><a href="../../facturacion.php"><img src="https://img.icons8.com/flat-round/28/000000/thick-long-right-arrow--v1.png" alt=""/> Timbrar otra Factura</a></td>
    <td width="1%">&nbsp;</td>

     <?php if($error <> 1)  { ?>
    <td width="47%" align="center"><a href="../../envio_correo_con_datos.php?idorden=<?php echo $cve; ?>">Enviar por Correo Electronico <img src="https://img.icons8.com/external-yogi-aprelliyanto-basic-outline-yogi-aprelliyanto/32/000000/external-email-file-and-folder-yogi-aprelliyanto-basic-outline-yogi-aprelliyanto.png"/></a></td>
	 <?php } ?>

  ...
  <tr>
    ...
    ?>
        ...
        <a href="../facturacion/timbrado/php/crear_xml.php?cve=<?php echo $_GET['idorden']; ?>"><img src="https://img.icons8.com/plasticine/50/000000/create-new.png"/> Crear XML 3.3</a>
          <?php echo "<br>"; ?>
        <a href="../facturacion/timbrado/php/crear_xml_4.0.php?cve=<?php echo $_GET['idorden']; ?>"><img src="https://img.icons8.com/plasticine/50/000000/create-new.png"/> Crear XML 4.0</a>
    ...
    ?>
      ...
      <p><a href="#" data-toggle="modal" data-target="#modal-emails" onclick="showMailModal()">Enviar por Correo Electronico <img src="https://img.icons8.com/external-yogi-aprelliyanto-basic-outline-yogi-aprelliyanto/32/000000/external-email-file-and-folder-yogi-aprelliyanto-basic-outline-yogi-aprelliyanto.png"/></a></p></td>
  ...
  function sendEmails(){
    ...
    for(email in addresses){
      if(addresses[email].selected){
        if(band == 0){
          addresses_list += addresses[email].value; // Si es el primer valor, no se pone coma
        }else{
          addresses_list += ', ' + addresses[email].value; // En caso contrario, se agrega una coma
        }
    ...
    if(selected_address.length != 0){
      console.log(selected_address.toString());
      console.log(ord_id);

      $.ajax({
        url: '../../envio_correo_con_datos__.php',
        data: {
          to: selected_address,
          idorden: ord_id
        },
        type: 'POST',
        timeout: 20000, // 20s de tiempo de respuesta
        success: (response) => {
          console.log(response);
          if(response == 'send'){
            Swal.fire({
              icon: 'success',
              text: 'Se envió el correo a '+addresses_list+' correctamente!',
              showConfirmButton: false,
              showCloseButton: true,
              position: 'top',
              allowOutsideClick: false
            });
            //window.parent.location.replace('https://elreydeltornillo.com/sit/facturacion/detalle_invoices.php?idorden=$fact&serie=$fact');
          }
        },

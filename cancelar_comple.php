...
include("conecta_facturacion.php");
$actualiza = mysqli_query($cone,"UPDATE `complementos` SET `estatus` = 'Cancelada' WHERE `complementos`.`uuid` = '$uuid';");
    ...
    if($respuesta == 'El comprobante será cancelado')
        ...
        $cance = $response->CancelaCFDIResult->anyType[7];

        echo $cance;

        if($cance == 'UUID Previamente cancelado')
        {

            echo "<p align='center'>Resultado de la cancelacion: ".$cance."</p>";

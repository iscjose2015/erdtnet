  ...
  <?php
        ...
        if($_GET['opc'] == 4){
         ...
         // echo "entro aqui".$bor;
          ...
          $inserta = mysqli_query($cone,"UPDATE `invoices` SET `metodo_pago` = '$for' WHERE `invoices`.`id` = '$idor';");
          $inserta = mysqli_query($cone,"UPDATE `invoices` SET `forma_pago` = '$met' WHERE `invoices`.`id` = '$idor';");
        //  header('location: detalle_invoices.php?idorden='.$_GET[idorden].'&serie='.$_GET[serie]);

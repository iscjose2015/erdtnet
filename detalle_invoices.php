  ...
  <?php
        ...
        if($_GET['opc'] == 4){
         ...
         echo $met;
          ...
          $inserta = mysqli_query($cone,"UPDATE `invoices` SET `pedimento` = '$pedi' WHERE `invoices`.`id` = '$idor';");
          $inserta = mysqli_query($cone,"UPDATE `invoices` SET `metodo_pago` = '$met' WHERE `invoices`.`id` = '$idor';");
          $inserta = mysqli_query($cone,"UPDATE `invoices` SET `uso` = '$uso' WHERE `invoices`.`id` = '$idor';");
       //   header('location: detalle_invoices.php?idorden='.$_GET[idorden].'&serie='.$_GET[serie]);
...
$re_receptor = mysqli_fetch_array($consulta_receptor);
  ...
  <tr>
    ...
    <td><?php echo $re_emisor['direccion']." CP ".$re_emisor[cp]; ?></td>
    <td><em><strong>Direccion</strong></em></td>
  ...
  <tr>
    ...
    if($re_orden['estatus'] == 'Timbrada' or $re_orden['estatus'] == 'Cancelada')
    {
    ...
    <form name="form1" method="get" action="detalle_invoices.php">
      <table width="100%" border="1">
    <tr>
    ...
    if ($for == '99') { $forma =   '<option value="99">99 Por Definir</option>'; }
    if ($for == '01') { $forma =   '<option value="01">01 Efectivo</option>';  }
    if ($for == '02') {$forma =    '<option value="02">02 Cheque nominativo</option>';  }
    if ($for == '03') {$forma =    '<option value="03">03 Transferencia electrónica de fondos</option>';  }
	if ($for == '15') {$forma =    '<option value="15">15 Condonacion</option>';  }
	if ($for == '17') {$forma =    '<option value="17">17 Compensación</option>';  }
    if ($for == '30') { $forma =   '<option value="30">30 Aplicacion de Anticipos</option>';  }
  ...
  <option value="99">99 Por Definir</option>
	...
	*/
    ...
    if ($met == 'PPD') { 
      ...
      echo ' <option value="PPD">PPD - PAGO EN PARCIALIDADES O DIFERIDO</option>'; }
      
    if ($met == 'PUE') { 
      
      echo '<option value="PUE">PUE - PAGO EN UNA SOLA EXHIBICION</option>';}
  ...
  <option value="PPD">PPD - PAGO EN PARCIALIDADES O DIFERIDO</option>
      ...
      <td>
        <label for="txtpedimento"></label>
        <input name="txtpedimento" type="text" id="txtpedimento" value="<?php echo $re_orden['pedimento']; ?>"></td>
    </tr>
    <tr>
    ...
    if ($us == 'G01') {  $men = 'G01 Adquisición de mercancías';}
    if ($us == 'G02') {  $men = 'G02 Devoluciones, descuentos o bonificaciones';}
    if ($us == 'G03') {  $men = 'G03 Gastos en general';}
    if ($us == 'P01') {  $men =  'P01 Por Definir';}
    if ($us == 'S01') {  $men = 'S01 Sin efectos fiscales';}
    if ($us == 'CP01') {  $men = 'Pagos';}

   ?>
  <select name="lbluso" id="lbluso" class="form-control" required="required" >
  <option value="<?php echo $us; ?>"><?php echo $men; ?></option>
  ...
  <p>
   ...
   <?php if ($re_orden['estatus'] == 'Por Emitir' ) { ?>
    <input type="submit" name="idorden" id="idorden" value="Actualizar">
    <?php }?>

    <input type="hidden" name="idorden" id="idorden" value="<?php  echo $_GET[idorden];?>">
    <input type="hidden" name="serie" id="serie" value="<?php  echo $_GET[serie];?>">
    <input type="hidden" name="txtid" id="txtid" value="<?php  echo $_GET[txtid];?>">
    <input type="hidden" name="opc" id="opc" value="<?php echo "4";?>">
  ...
  <table width="100%" border="1">
   ...
   if($re_orden['estatus'] == 'Por Emitir') { echo "No se puede cancelar por que no se ha timbrado";}
    ...
    if($re_orden['estatus'] == 'Timbrada')
    {
      ...
      <a href="cancelar.php?folio_fiscal=<?php echo $re_orden['uuid']; ?>"><img src="https://img.icons8.com/cotton/64/000000/cancel--v1.png"/> Cancelar directo Motivo 02, error en el llenado</a>
      <?php
        }
      ...
      <a href="cancelar_4.0.php?folio_fiscal=<?php echo $re_orden['uuid']; ?>"><img src="https://img.icons8.com/cotton/64/000000/cancel--v1.png"/> Cancelar por motivo 02, error en el llenado</a>
...
<a href="cancelar_4.0_motivo_03.php?folio_fiscal=<?php echo $re_orden['uuid']; ?>"><img src="https://img.icons8.com/cotton/64/000000/cancel--v1.png"/> Cancelar por motivo 03, NO se completo la operacion </a>
